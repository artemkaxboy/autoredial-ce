...
class DbHelper {
  ...
  private static final String CREATE_REJECT = "CREATE TABLE IF NOT EXISTS " + TABLE_REJECTED
      + " ( " + FIELD_ID + " INTEGER NOT NULL PRIMARY KEY, " + FIELD_NUMBER + " TEXT NOT NULL, "
      + FIELD_NUMBER_CLEAR + " TEXT NOT NULL );";
  ...
  static void addRejected(Context context, ArrayList<String> items) {
  ...
  static void updateRejected(Context context, ArrayList<String> items, boolean[] checked) {
    SQLiteDatabase db = getDB(context);
    if (db == null) {
      Toast.makeText(context, "Error", Toast.LENGTH_LONG).show();
      return;
    ...
    for (int i = 0; i < items.size(); i++) {
      if (checked[i]) {
        if (isInRejected(context, items.get(i)) > 0) {
          continue;
        }
        ContentValues value = new ContentValues();
  ...
  static List<MyContact> getListRejected(Context context) {
      ...
      if (c.moveToFirst()) {
        do {
          ...
          if (contact.loadByNumber(context, number)) {
            ...
            db.delete(TABLE_REJECTED, FIELD_ID + " = " + c.getLong(c.getColumnIndex(FIELD_ID)),
                null);
          }
        } while (c.moveToNext());
      }
  ...
  static long isInRejected(Context context, String number) {
    long ret = -1;
    ...
    Cursor c = db
        .query(TABLE_REJECTED, null, FIELD_NUMBER_CLEAR + " LIKE '" + number + "'", null, null,
            null, null);
    if (c != null) {
      if (c.moveToFirst()) {
